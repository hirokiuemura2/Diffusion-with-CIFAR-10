...
def main():
    channel_size = 24 # Change Channel Count Here
    device = "cuda" if torch.cuda.is_available() else "mps" if torch.backends.mps.is_available() else "cpu"
    net  = UNet(mid_channels=channel_size).to(device) 
    net.load_state_dict(torch.load(f"model_weights_shallow_{channel_size}Channels.pth", weights_only=True, map_location=device)) # Change model_weights filename if needed
    net.eval()
    ...
    t = torch.tensor([10] * x.size(0), device=x.device)
    noised_x, noise = addNoise(x, t, betaSchedule)
    newImages = noised_x.clone()
    

    denoiseCount = 10  # How many denoising steps to show
